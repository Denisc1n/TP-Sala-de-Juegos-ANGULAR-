...
export class SimonDiceComponent implements OnInit {
  ...
  deshabilitarBotones(valor: boolean) {
    ...
    (<HTMLInputElement>(
      document.getElementById("btn-amarillo")
    )).disabled = valor;
  ...
  showBoton(boton: number, time: number, soundLoser: boolean): boolean {
    if (soundLoser) this.playSound(5);
    else this.playSound(boton);

    switch (boton) {
      case 0:
        document.getElementById("btn-verde").style.backgroundColor =
          "rgba(40,167,69,0.5)";
        document.getElementById("btn-rojo").style.backgroundColor =
          "rgba(220,53,69,0.5)";
        document.getElementById("btn-amarillo").style.backgroundColor =
          "rgba(255,193,7,0.5)";
        document.getElementById("btn-azul").style.backgroundColor =
          "rgba(0,123,255,0.5)";
        return false;
      case 1:
        document.getElementById("btn-verde").style.backgroundColor =
  ...
  botonApretado(botonNumber: number) {
    ...
    if (
      this.nuevoJuego.secuencia.length == this.indiceUsr + 1 &&
      this.nuevoJuego.gano
    ) {
      this.contador++;
